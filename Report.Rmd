...
Here below you can see a leaflet representation of the distribution of people in the whole world:
...
![](Images/pop_number2.png){ width=50% } ![](Images/pop_growth2.png){ width=50% }
...
![](Images/continent_distribution.png){ width=70% ref="refe"}
...
I decided to move forward my analysis combining this datasets to some additive ones I found on the World Bank Open Data at https://data.worldbank.org/, where I decided ti download the collections of data regarding population amount, birthrate and deathrate (both over 1000 people) and the Gross Domestic Product of a Country; those datasets contain measures from 1960 to 2016 for (quite) every Country in the world, but they show missing data sometimes.<br />
...
In the columns with the years as names, are then contained the actual data.
...
As you can see, I did not had to count because also the "World" data are a row of our dataset.<br />
...
I created then the leaflet interactive map: to make it I used the code you can find inot "leaflet_map.R" script. For it, I had to perform other data arrangements because some Country's names where not exact for leaflet, so it did not show some data at all: for example, instead of "United States" it was expected "United States of America", so I had to check manually for a lot of names.
...
[2] https://www.ecology.com/population-estimates-year-2050/ (early ages)<br />
